...
def generate_similar(payload: dict):
    ...
    try:
        resp = httpx.post(ollama_url, json={"model": "qwen2.5:7b", "prompt": prompt, "stream": False}, timeout=120)
    ...
    try:
        ...
        return JSONResponse(content={"error": "模型返回的 JSON 解析失败，再试一次"}, status_code=500)
...
def ai_explain(payload: dict):
    ...
    prompt = (
        ...
        f"【学生答的】{user_ans}\n"
        + book_section +
        "\n\n请用 3~5 句话：①一句话说这道题考什么知识点；②如果答错了，点破他错在哪；③再用大白话把正确思路讲一遍。如果上面给了老师笔记，要结合笔记里的重点和易错点讲。"
    )
    ollama_url = "http://127.0.0.1:11434/api/generate"
    try:
        resp = httpx.post(ollama_url, json={"model": "qwen2.5:7b", "prompt": prompt, "stream": False}, timeout=60)
        data = resp.json()
        return JSONResponse(content={
        ...
        return JSONResponse(content={"ai_explain": "", "error": "调用 Ollama 失败：" + str(e)}, status_code=500)
