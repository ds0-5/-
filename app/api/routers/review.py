...
def review(payload: dict):
    ...
    if not uid or not qid:
        return JSONResponse(content={"error": "缺少 user_id 或 qid"}, status_code=400)
    ...
    return JSONResponse(content={
        ...
        "next_due": row["due"], "ef": round(row["ef"], 2),
    })

# 取今天该复习的题（复习队列）
@router.get("/due")
def due_cards(user_id: str = "", course: str = "", chapter: str = "", limit: int = 50, include_new: bool = False):
    if not user_id:
        return JSONResponse(content={"error": "缺少 user_id"}, status_code=400)
    today = datetime.now().isoformat(timespec="seconds")
    conn = sqlite3.connect(settings.db_path); conn.row_factory = sqlite3.Row
    base = ("SELECT s.qid AS qid, s.due AS due, s.interval AS interval, s.reps AS reps, "
    ...
    args = [user_id, today]
    if course:
        base += " AND q.course=?"; args.append(course)
    if chapter:
        base += " AND q.chapter=?"; args.append(chapter)
    if include_new:
        new = ("SELECT q.id AS qid, '' AS due, 0 AS interval, 0 AS reps, "
               "q.question, q.options, q.answer, q.chapter, q.knowledge_point, q.type "
               "FROM questions q WHERE q.type NOT IN ('fill','apply','comprehensive') "
               "AND q.id NOT IN (SELECT qid FROM srs WHERE user_id=?)")
        nargs = [user_id]
        if course:
            new += " AND q.course=?"; nargs.append(course)
        if chapter:
            new += " AND q.chapter=?"; nargs.append(chapter)
        base = base + " UNION " + new
        args = args + nargs
    base += " ORDER BY due ASC LIMIT ?"; args.append(limit)
    rows = conn.execute(base, args).fetchall(); conn.close()
    out = []
    for r in rows:
        out.append({
            "qid": r["qid"], "due": r["due"], "interval": r["interval"], "reps": r["reps"],
            "question": r["question"],
            "options": json.loads(r["options"]) if r["options"] else {},
            "answer": r["answer"], "chapter": r["chapter"],
            "knowledge_point": r["knowledge_point"], "type": r["type"],
        })
